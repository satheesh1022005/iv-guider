  ...
  return (
    <div>
      <div  className="top-menu">
        <span><button><input type="checkbox" name={step.title}></input></button>{step.title}</span>
        <span className="Symbol" onClick={toggleExpand}>{expanded ? '-' : '+'}</span>
      </div>
      {expanded && step.details && (
        <div style={{ marginLeft: '20px' }}>
          {step.details.map((subStep) => (
            <div key={subStep.sub_step} className="nested-menu">
              <strong>{subStep.title}</strong>
              <p>{subStep.description}</p>
              {subStep.details && <SubStep subSteps={subStep.details} />}
              {!subStep.details && <div className='sections'>
                <section>
                    <a href={subStep.link} target='_blank' rel="noreferrer">
                    ...
                    <p className="icon-label">Reference</p>
                    </div>
                    </a>
                </section>
                <section>
                    ...
                    <div className="icon-container">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white" class="bi bi-file-earmark-arrow-down-fill" viewBox="0 0 16 16">
